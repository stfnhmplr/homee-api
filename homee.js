...
class Homee extends EventEmitter {
            ...
            request.post(options, (err, res, body) => {
                if (!err && res.statusCode === 200) {
                    ...
                    debug(
                        ...
                        new Date(this._expires).toISOString()
                    );
                    resolve(this._token);
    ...
    _handleAttributeChange(attribute) {
        ...
        if (this._nodes.length) {
            ...
            const attributeIndex = this._nodes[nodeIndex].attributes.findIndex(
                a => a.id === attribute.id
            );
    ...
    setValue(device_id, attribute_id, value) {
        debug(
            'trying to set %d as target_value for attribute #%d (device #%d)',
            value,
            attribute_id,
            device_id
        );
        ...
        switch (type) {
            ...
            case 'attribute':
                const attribute = [].concat
                    .apply([], this._nodes.map(n => n.attributes))
                    .find(a => a.id === id);
